  ...
  const handleUsePhoto = () => {
    if (photoFile && currentPhoto) {
      if (photoType === 'start') {
        ...
        setClockedIn(true);
        setActiveStep(3);
        showSnackbar('Clock-in successful! 🚀', 'success');
      } else {
        setCurrentTrip(prev => ({
          ...prev,
          endPhoto: currentPhoto,
          endPhotoFile: photoFile
        }));
      }
  ...
  const completeClockOut = async () => {
    try {
      ...
      if (currentTrip?.endPhotoFile) {
        try {
          const result = await uploadPhoto(currentTrip.endPhotoFile, currentTrip.tripId, 'end');
          endPhotoUrl = result.photoUrl;
          showSnackbar('End photo uploaded successfully', 'success');
        } catch (uploadError) {
          console.error('Failed to upload end photo:', uploadError);
          showSnackbar('Photo upload failed, but clock-out will proceed', 'warning');
        }
      }
      ...
      if (!response.ok) throw new Error('Failed to submit clock-out data');
      ...
      showSnackbar('Clock-out successful! Trip completed. 🎉', 'success');
    } catch (error) {
            ...
            >
              <Box sx={{ textAlign: 'center', py: 2 }}>
                ...
                <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
                  Vehicle: <strong style={{ color: '#6366F1' }}>{formData.vehicleNo}</strong>
                </Typography>
                ...
                <TextField
                  ...
                  InputProps={{
                    startAdornment: <SpeedIcon sx={{ mr: 1, color: '#6366F1' }} />,
                  }}
                  sx={{ 
                    mb: 3, 
                    maxWidth: 300,
