...
<TableBody>
  {filteredSchedules.length > 0 ? (
    filteredSchedules
      ...
      .map((schedule, index) => (
        <TableRow key={schedule.vehicle_id}>
          <TableCell>{page * rowsPerPage + index + 1}</TableCell>

          {/* Bus Name */}
          <TableCell>{schedule.vehicle_no}</TableCell>

          {/* Assigned Time */}
          <TableCell>
            {schedule.bus_number || 'Not Assigned'} {/* replace with actual assigned time if available */}
          </TableCell>

          {/* Assigned By */}
          <TableCell>
            {schedule.supervisor_flag === 1 ? (
              <Typography
                variant="body2"
                sx={{
                  color: '#fff',
