...
const fetchAssignments = async () => {
  ...
  try {
    const response = await axios.get(`${API_BASE_URL}/supervisor/driver-list`, {
    ...
    const driverData = drivers.map((driver, index) => ({
      id: driver.user_id,
      driver_id: driver.user_id || 'N/A',
      driver_name: driver.name || 'N/A',
      emp_id: driver.emp_id || 'N/A',
       vehicle_no: driver.vehicle_no || 'N/A', 
      assignment_date: new Date().toISOString().split('T')[0],
      created_at: new Date().toISOString(),
      notes: `Driver record for ${driver.name}`,
      status: 'Active',
    }));
    ...
    if (driverData.length === 0) {
      showSnackbar('No drivers found', 'info');
    }
  } catch (error) {
    ...
    if (error.response?.status === 401) {
      ...
      const sampleDrivers = [
        { id: 'DRV001', driver_name: 'John Doe', emp_id: 'EMP001', vehicle_no: 'N/A', assignment_date: '2024-01-15', created_at: '2024-01-15T10:00:00Z', notes: 'Sample driver record', status: 'Active' },
      ];
      setAssignments(sampleDrivers);
      showSnackbar('Using sample data - Configure API endpoint', 'info');
    } else {
  ...
  const handleEditAssignment = (assignment) => {
    ...
    navigate('/assign-driver', { state: { assignment } });
  };
  ...
  try {
    // 🔹 Call backend route to set status=0 using user_id
    await axios.put(
      `${API_BASE_URL}/supervisor/update-driver-status?user_id=${selectedAssignment.driver_id}`,
      { status: 0 },
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    // 🔹 Update frontend table to reflect change
    setAssignments((prev) =>
      prev.map((a) =>
        a.driver_id === selectedAssignment.driver_id
          ? { ...a, status: 'Deleted', flag: 0 }
          : a
      )
    );

    showSnackbar('Driver status updated to inactive!', 'success');
    setDeleteDialog(false);
    setSelectedAssignment(null);
  } catch (error) {
    console.error('Error updating driver status:', error);
    showSnackbar('Error updating driver status', 'error');
  }
};
  ...
  const handleExport = () => {
    ...
    try {
      ...
      link.setAttribute('download', `driver_assignments_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const filteredAssignments = assignments.filter(assignment =>
    ...
    assignment.driver_id?.toString().includes(searchTerm) ||
        ...
        >
          ...
          <DialogContent>
            <Typography>
              ...
              <strong>{selectedAssignment?.driver_name}</strong> on vehicle{' '}
            ...
            {selectedAssignment && (
              <Box sx={{ mt: 2 }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 3 }}>
                  ...
                  <Typography><strong>Driver Name:</strong> {selectedAssignment.driver_name}</Typography>
                  <Typography><strong>Driver ID:</strong> {selectedAssignment.driver_id}</Typography>
...
export default ListAssignDriver;